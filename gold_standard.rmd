...
elp <- read.csv("elp_new.csv")
#ws353_subs <- read.table("sim353_subs2vec.txt", header = TRUE)
#colnames(ws353_subs)[3] <- "subs353"
#ws999_subs <- read.table("sim999_subs2vec.txt",header = TRUE)
#colnames(ws999_subs)[3] <- "subs999"
for (col in colnames(elp)[2:8]){
  if (col == "Semantic_Neighbors"){
    elp[,col] <- as.numeric(gsub(",","",elp[,col]))
  }
  else{
    elp[,col] <- as.numeric(elp[,col])
  }
}
...
for (i in 1:nrow(word_pairs)){
  ...
  row = filter(R123.dat, R123.dat$cue == w1 & R123.dat$response == w2)[,c("cue", "response","R123.Strength")]
  if (nrow(row) == 1){
    colnames(row) <- c("word1","word2","R123_sw")
    temp <- row
...
sw_ws353$word1_semN<- NA
sw_ws353$word2_semN <- NA
for (i in 1:nrow(sw_ws353)){
  ...
  sw_ws353$word1_semN[i] = ifelse(nrow(filter(elp,elp$Word == w1)) ==1, filter(elp,elp$Word == w1)$Semantic_Neighbors,NA)
  sw_ws353$word2_semN[i] = ifelse(nrow(filter(elp,elp$Word == w2)) ==1, filter(elp,elp$Word == w2)$Semantic_Neighbors,NA)
}
return(sw_ws353)
}


sw_ws353.sim <- get_sw353("sim353_sim")
sw_ws353.rel <- get_sw353("sim353_rel")

#write.csv(sw_ws353.sim, "sim353_sim.csv")
#write.csv(sw_ws353.sim, "sim353_rel.csv")
...
R2_sw353 <- get_sw353("sim353") #both rel and sim
R2_sw353_sim <- get_sw353("sim353_sim")
R2_sw353_sim$Type <- "Similar"
R2_sw353_rel <- get_sw353("sim353_rel")
R2_sw353_rel$Type <- "Related"
R2_sw353_split <- rbind(R2_sw353_sim,R2_sw353_rel) #sw353 with rel and sim distinguished

R2_sw353_split <- R2_sw353_split %>% group_by(Type)%>% mutate(additional_R2 = R2 - R2_sw353_split[R2_sw353_split$Model == "Baseline" &R2_sw353_split$Type == Type,]$R2) #gets additional R2 (subtract R2 explained by baseline model)
...
sw_ws999$word1_semN<- NA
sw_ws999$word2_semN <- NA
for (i in 1:nrow(sw_ws999)){
  ...
  sw_ws999$word1_semN[i] = ifelse(nrow(filter(elp,elp$Word == w1)) ==1, filter(elp,elp$Word == w1)$Semantic_Neighbors,NA)
  sw_ws999$word2_semN[i] = ifelse(nrow(filter(elp,elp$Word == w2)) ==1, filter(elp,elp$Word == w2)$Semantic_Neighbors,NA)
}
